...
class CLAM_SB(nn.Module):
    def __init__(self, gate = True, size_arg = "small", dropout = False, k_sample=8, n_classes=2,
        ...
        self.size_dict = {"small": [1024, 512, 256], "big": [1024, 512, 384]} #small big 중에 선택
        size = self.size_dict[size_arg] # size배열 저장

        fc = [nn.Linear(size[0], size[1]), nn.ReLU()] #fully connected layer 구성
        ...
        # fc === [nn.Linear(size[0], size[1]), nn.ReLU(), attention_net]
    ...
    def forward(self, h, label=None, instance_eval=False, return_features=False, attention_only=False):
        ...
        # A -> attention map h -> 원본 데이터 
        M = torch.mm(A, h)
        logits = self.classifiers(M)  # M -> Self-Attemtion feature maps
