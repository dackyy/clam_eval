...
if args.task == 'task_1_tumor_vs_normal':
    args.n_classes=2
    dataset = Generic_MIL_Dataset(csv_path='/ai-data/patho/WORKSPACE/dkjeong/CLAM/data/data.csv',
                                  data_dir=os.path.join(
                                      args.data_root_dir, '/ai-data/patho/WORKSPACE/dkjeong/CLAM/data'),
                            shuffle = False, 
                            print_info = True,
    ...
    args.n_classes = 4
    dataset = Generic_MIL_Dataset(csv_path='/ai-data/patho/WORKSPACE/dkjeong/DATA/MOC_Kidney/us.csv',
                                  data_dir=os.path.join(args.data_root_dir),
                                  shuffle=False,
                                  print_info=True,
                                  label_dict={'Normal': 0, 'RCC_clear': 1,
                                              'RCC_chromophobe': 2, 'RCC_papillary': 3},
                                  patient_strat=False,
                                  ignore=[])
...
#                                   ignore=[])
    ...
    args.n_classes = 2
    dataset = Generic_MIL_Dataset(csv_path='dataset_csv/Kidney_list.csv',
                                  data_dir=os.path.join(
                                      args.data_root_dir, 'features'),
                                  shuffle=False,
                                  print_info=True,
...
if __name__ == "__main__":
    ...
    for ckpt_idx in range(len(ckpt_paths)):
        ...
        if args.inference:
            print('end!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
            pred = eval(split_dataset, args, ckpt_paths[ckpt_idx])
            all_auc, all_acc = -1, -1
            print(pred)
        else:
